...
% a=y(4*(i-1)+1:4*i,:);
% [m, p, DistHist]=vqsplit(a',K);%矢量量化，m为量化后的聚类中心
%[C0, dst]=VQIndex(a',m) ;%把本训练样本按照前面聚类结果量化，C0为量化后的码本
% [C0, dst]=VQIndex(a',m) ;%把本训练样本按照前面聚类结果量化，C0为量化后的码本
% data = y(12*(j-1)+1:12*j,:);
% end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%测试样本量化
%     x=z(10,:)';
%      y=hmmfeatures(x,N,deltaN,M,Q1); 
%      [yc,C1,errlog]= kmeans1(y,K,maxiter);
%%%%%%%%% %    [m, p, DistHist,minIndx]=vqsplit(a',K);%矢量量化，m为量化后的聚类中心
%%%%%%%%%% %    [C0,dst]=VQIndex(a',m);
...
%     0.2849    1.0000    0.5678    0.0013    0.2633    0.8849    0.6764         0    0.2604    0.8853    0.5723    0.0085    0.2711    0.8578    0.6034    0.0044;
%     0.1130    1.0000    0.2122    0.0066    0.1156    0.9439    0.2473  0.0032    0.1093    0.9889    0.2217    0.0099    0.1278    0.9680  0.2451         0;
%     0.801    0.04   0.26       0.0004     0.15      0.75      0.5        0.0005     0.16     0.89    0.2        0.1        0.92      0.57    0.001        0.001;
%     
%     ];
% 
% y1=[%%%%%%%%%%无故障
...
%  %对新的信号得到他的码本
% %[C1, dst1]=VQIndex(y1(1,:)',m) ;%对新的信号得到他的码本
for ii=1:28
  a=y1(ii,:);
  [partition,codebook] = lloyds(a,2^4);
  N=length(partition)+1;  
