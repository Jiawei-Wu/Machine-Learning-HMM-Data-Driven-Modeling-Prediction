function [S, stats] = gibbstrain(Y, hypers, numb, nums, numi, S0)


%function[radia_int_vec, tr_mat, emit_mat] = gibbstrain(temper_int_vec,
%num_hidden_states)
...
% 
%   Input Parameters: - Y: training sequence of arbitrary length,###任意长度的序列
...
%             field H,
%   - numb: the number of burnin iterations,###开始阶段迭代次数 - nums: the number
%   of samples to output,###输出的样本数量 - numi: the number of sampling,
%   iterations between two samples, - S0: is the initial assignment to the
%   sequence.###序列的初始值
% 
%   Output Parameters: - S: is a cell array of sample structures where each
%   sample contains the
...
L = size(hypers.H,2);               % # of emission symbols.###可见种类
%###hypers.H  这个矩阵的列数

sample.S = S0;
sample.K = max(S0);  %#####隐态是怎么确定的！！
...
while iter <= (numb + (nums-1)*numi)%###迭代500次

    % Compute the empirical emission matrix.####经验发射矩阵
    % E(i,j) = number of emmisions of symbol l from state i.
    E = zeros(sample.K, L);%##发射矩阵维度是： 状态数*（可见种类）
    for t=1:T
        E(sample.S(t), Y(t)) = E(sample.S(t), Y(t)) + 1;
    end%####统计出现的次数 
    %####暂且理解为一个样本是一对（状态+观察符号）
    
    % Compute the empirical transition matrix.###经验转移矩阵
    ...
    for t=1:T
        ...
        if sample.S(t) > sample.K
            ...
            N(sample.S(t), :) = 0;%####转移概率矩阵扩大一维
            E(sample.S(t), :) = 0;%####发射矩阵增大一行
            
            % Extend Beta. Standard stick-breaking construction stuff
            b = betarnd(1, sample.gamma);%###贝塔分布的随机数   一个随机数
    ...
    % Recompute the number of states.（上面的步骤中已经更改了隐状态的值 和范围）
    zind = sort(setdiff(1:sample.K, unique(sample.S)));%###集合的差值 升序 
